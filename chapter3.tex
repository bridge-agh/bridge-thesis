...
\begin{itemize}
  \item \textbf{Bridge Core} -- główna logika gry w~brydża; stworzona
        w~języku Python,

  \item \textbf{Wirtualny asystent} -- algorytm sztucznej inteligencji; stworzony
        w~języku Python z~wykorzystaniem biblioteki JAX~\cite{JAX},

  \item \textbf{Aplikacja webowa} -- główny interfejs dla użytkowników,
        aby móc rozgrywać grę; stworzone przy zastosowaniu
        frameworków Next.js~\cite{NextJS} i~Tailwind.css w~językach HTML, \mbox{Javascript}
        oraz CSS,

  \item \textbf{Serwer gry} -- centralny serwer, którego zadaniem jest
        łączenie graczy do sesji, przeprowadzanie gier i~zarządzaie
        w~nich asystentem; stworzony przy użyciu języka Scala~\cite{Scala}
        \mbox{i~frameworku} Akka~\cite{Akka},

  \item \textbf{Bridge Engine} -- interfejs gry brydża; stworzony
        z~użyciem frameworka Three.js~\cite{ThreeJS} i~języka Javascript,
...
w~języku Scala. Wykorzystany został potencjał
